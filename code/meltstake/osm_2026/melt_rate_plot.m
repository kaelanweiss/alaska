% Script to plot melt rate results
%
% KJW
% 11 Feb 2025
...
load ../glacier_grl_2025/glacier_clrs
...
for i = size(idx_glac,2):-1:1
    ms = marker_scale;
    if strcmp(smbs{i},'square')
        ms = ms*1.3;
    end
    h1(i) = scatter(ax(1),u(idx_glac(:,i)),m(idx_glac(:,i)),ms,T(idx_glac(:,i))-Tf,smbs{i},'filled','markeredgecolor','k');
end
cmocean('sol',ax(1))
...
plot(ax(2),[0 10],1.6*[0 10],'k--','linewidth',lw)
...
plot(ax(3),[0 10],4.4*[0 10],'k--','linewidth',lw)
clear h3
...
xlabel(ax(3),'m_{3eqn} [m/day]','fontsize',fs)
...
lgd.Position([1 2]) = [ax(2).Position(1) sum(ax(2).Position([2 4]))+.02];
...
% line labels
for i = 2:3
    text(ax(i),0.9,0.85,'1:1','units','normalized','fontsize',fs-1,'rotation',45,'horizontalalignment','center','verticalalignment','middle')
end
text(ax(2),0.63,0.9,'1.6:1','units','normalized','fontsize',fs-1,'rotation',atand(1.5),'horizontalalignment','center','verticalalignment','middle')
text(ax(3),0.13,0.9,'4.4:1','units','normalized','fontsize',fs-1,'rotation',atand(4.4),'horizontalalignment','center','verticalalignment','middle')
