...
model_lbls = {'3-equation','convective','modified 3-eqn (x)','bulk iceberg','3-eqn (coarse)','modified 3-eqn'};
title_lbls = {'','','(x)','(d)','(c)','(e)'};
model_num = 6;
...
text(ax1,.02,.98,['(a) ' model_lbls{model_num}],'units','normalized','fontsize',fs,...
        'verticalalignment','top','horizontalalignment','left','BackgroundColor','w')
...
% labels
xlabel(ax1,'$\overline{\textsf{U}_\textsf{max}}$ \textsf{[m/s]}','fontsize',fs,'interpreter','latex')
ylabel(ax1,'$\overline{\textsf{T}}$ \textsf{[K]}','fontsize',fs,'interpreter','latex')
cbar.Label.String = '$\textsf{m}_\textsf{obs}/\textsf{m}_\textsf{mod}$';
cbar.Label.FontSize = fs-1;
cbar.Label.Interpreter = 'latex';
cmocean('balance','pivot',1)

% contour text label
text(0.108,7.2,'$\overline{\textsf{U}_\textsf{max}}\cdot\overline{\textsf{T}}=$\textsf{const}.','rotation',-45,...
    'verticalalignment','middle','horizontalalignment','center','interpreter','latex')
...
ax2 = axes(fig1,'position',axgridpos(1,2,2,pad,shift) + [0 -.02 0 0]);
hold on
box on
...
ylabel('m_{obs} [m/day]','fontsize',fs)
xlabel('m_{mod} [m/day]','fontsize',fs)
...
if ~any(model_num == [4 6])
    lgd.Position = lgd.Position + [.015 -.03 0 0];
else
    lgd.Position = lgd.Position;% + [.015 -.03 0 0];
end
...
xlabel('$\textsf{log}_\textsf{10}\Big(\frac{\textsf{m}_\textsf{obs}}{\textsf{m}_\textsf{mod}}\Big)$','fontsize',fs-2,'interpreter','latex')
