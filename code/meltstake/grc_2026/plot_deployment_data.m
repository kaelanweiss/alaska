...
% vel magnitude
plot(ax(3),adcp.burst.time,hannFilter(umax,8*2),'-','color',vel_clrs(1,:),'linewidth',lw)
...
p_lbls(1).String = 'u (horzizontal along-ice)';
...
% colobar
cbar = colorbar(ax(2),'position',cbarpos(ax(1:2),.02,.02));
...
% axes limits
linkaxes(ax,'x')
xlim(ax(1),[seg_tbl.Start(1) seg_tbl.End(3)]+0.1*minutes([-1 1]))
