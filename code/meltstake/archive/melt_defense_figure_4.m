...
sections = [8 29];% 3 29 9];
section_lbls = {'Crossflow (A)','Plume (B)','Wave (C)','Eddy (D)'};
section_lbls = {'Crossflow (A)','Wave (C)'};
...
for i = 1:ns    
    ...
    % smoothing
    k_T = round(hann_T_dt/seconds(diff(T.time(1:2))));
    k_u = hann_u_dt*adcp.burst.samplerate;
    % T
    for j = 1:size(T.T,2)
...
panel_lbls = {'Temperature (T)','Velocity magnitude (U)','horz vel (u)','vert vel (w)'};
y_lbls = {'$^\circ$C','m/s',{''},'',{''}};

% manual lims
T_lim = [3 8];
vel_lim = [0 0.2];
pc_lims = [.15 .15 .15 .15]+.001;%-.049;
...
for i = 1:ns
    % plot
    % T
    ax(nrows*(i-1)+1) = axes(fig,'position',axgridpos(nrows,ns,nrows*(i-1)+1,pxi,pyi,pxo,pyo,'flip'));
    hold on
    for j = 1:3
        plot(minutes(T_all{i}.time-t0(i)),T_all{i}.T(:,j),'linewidth',lw,'color',T_clrs(:,j))
    end
    title({sprintf('%s',section_lbls{i}),...
        strrep(sprintf('melt rate: %.1f$\\pm$%.1f m/day',m(i),m_ci(i)),'m$0.','m$.')},'fontsize',fs+4)
    ylim(T_lim)
    box on

    % vel scale
    adcp_time = minutes(adcp_all{i}.burst.time-t0(i));
    ax(nrows*(i-1)+2) = axes(fig,'position',axgridpos(nrows,ns,nrows*(i-1)+2,pxi,pyi,pxo,pyo,'flip'));
    plot(adcp_time,hannFilter(adcp_all{i}.burst.vel_scale,8),'k','linewidth',lw)
    ylim(vel_lim)

    % u
    ax(nrows*(i-1)+3) = axes(fig,'position',axgridpos(nrows,ns,nrows*(i-1)+3,pxi,pyi,pxo,pyo,'flip'));
    pcolor(adcp_time,adcp_all{i}.burst.range,-adcp_all{i}.burst.vel_ice(:,:,1)')
    shading flat
    cmocean('bal')
    clim(pc_lims(i)*[-1 1])
    ylim([-.1 rmax_all])
    patch([0 0 minutes(t_width(i)) minutes(t_width(i))],-.1*[1 0 0 1],0.8*[1 1 1])

    % w
    ax(nrows*(i-1)+4) = axes(fig,'position',axgridpos(nrows,ns,nrows*(i-1)+4,pxi,pyi,pxo,pyo,'flip'));
    pcolor(adcp_time,adcp_all{i}.burst.range,adcp_all{i}.burst.vel_ice(:,:,2)')
    shading flat
    cmocean('bal')
    clim(pc_lims(i)*[-1 1])
    ylim([-.1 rmax_all])
    patch([0 0 minutes(t_width(i)) minutes(t_width(i))],-.1*[1 0 0 1],0.8*[1 1 1],'linestyle','-','clipping','on')

    % w
%     ax(nrows*(i-1)+5) = axes(fig,'position',axgridpos(nrows,ns,nrows*(i-1)+5,pxi,pyi,pxo,pyo,'flip'));
...
% colorbars
cbu = colorbar(ax(nrows*ns-1));
cbw = colorbar(ax(nrows*ns));
cbu.Position = cbarpos(ax(nrows*ns-1),.005,.02);
cbw.Position = cbarpos(ax(nrows*ns),.005,.02);
...
for i = 1:nrows
    for j = 1:ns
        txt = text(ax(i+nrows*(j-1)),.025,.97,panel_lbls{i},'units','normalized','fontsize',fs,...
