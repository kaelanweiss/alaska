...
x_data = {u,T,S};
x_ci = {u_ci,T_ci,S_ci};
x_lbls = {'m/s','$^\circ$C','psu'};
c_data = [2,1,2];
c_cmap = {'-solar','tempo','-solar'};
c_lbls = {'$T$ [$^\circ$C]','$U$ [m/s]','$T$ [$^\circ$C]'};
clrs = [colors(1); colors(3); colors(2)];
pnl_lbls = {'melt vs. $U$','melt vs. $T$','melt vs. $S$'};
for i = 1:3
    axes(figure(22),'position',axgridpos(3,1,i,.1,.1,.15,.1)-[.04 0 0 0])
    hold on
    ...
    text(.01,.98,['(' char(97+i) ') ' pnl_lbls{i}],'units','normalized','fontsize',fs,...
        'verticalalignment','top','horizontalalignment','left')
    cbar = colorbar;
