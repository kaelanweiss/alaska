...
for i = 1:4
    % axis
    ax(i) = axes(figure(3),'position',axgridpos(2,2,i,pxi,pyi,pxo,pyo));
    hold on
    box on

    % plot
    plot([0 15],1*[0 15],'k-')
    %plot([0 2.5],3*[0 2.5],'k-')
...
for i = 1:4
    for j = 1:length(sections)
        h(j) = plot(ax(i),m_mean(sections(j),i),m_obs(sections(j)),'ko','markersize',6,...
            'markerfacecolor',colors(j+1),'marker',mkrs{j});
    end
end
legend(ax(2),h,lgd_lbls,'location','southeast','fontsize',fs,'color','w')
