...
for i = 1:n_deps
    ...
    for j = 1:n_wndws
        ...
        [m1,Tb1,Sb1] = solve3Eqn(U1_scale,T_scale,S_scale,0);
        [m2,Tb2,Sb2] = solve3Eqn(U2_scale,T_scale,S_scale,0);
        m1 = m1*86400;
        m2 = m2*86400;
        ...
        fprintf('%.2f (%.2f) m/day\n%.2f (%.2f) m/day\n',mean(m1,'omitnan'),std(m1,'omitnan'),mean(m2,'omitnan'),std(m2,'omitnan'))
        
        % "save" output
        m_all(find(isnan(m_all(:,1)),1),:) = [mean(m1,'omitnan'),std(m1,'omitnan'),mean(m2,'omitnan'),std(m2,'omitnan')];
    end
