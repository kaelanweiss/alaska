% Script to create figure 1 for the melt rate defense. This is a
...
% calculate magnitude
vel_mag_smth = vecnorm(vel_smth(:,:,1:3),2,3);
...
% panel labels
panel_lbls = {'Velocity (U)','Temperature (T)','Salinity (S)',''};
...
ax(4).Position(4) = ax(3).Position(2) - ax(4).Position(2) - pyi - .05;
hold on
...
for i = 1:4
    text(ax(i),.01,.98,sprintf('%s',panel_lbls{i}),'units','normalized','fontsize',fs,...
