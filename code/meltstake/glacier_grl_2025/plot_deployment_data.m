...
% load velocity, T, S
u_seg = msTable2Vector(seg_tbl.u_max);
T_seg = msTable2Vector(seg_tbl.T);
S_seg = msTable2Vector(seg_tbl.S);
...
for i = 1:nseg
    if i == 1
        h6(1) = plot(ax(6),[seg_tbl.Start(i) seg_tbl.End(i)]+0.7*minutes([1 -1]),m(i)*[1 1],'-','linewidth',1*lw,'color',0.3*[1 1 1]);
    else
        plot(ax(6),[seg_tbl.Start(i) seg_tbl.End(i)]+0.7*minutes([1 -1]),m(i)*[1 1],'-','linewidth',1*lw,'color',0.3*[1 1 1])
    end
end
% mean 3eqn
% h6(4) = plot(ax(6),t_melt+minutes(0.),m_3eqn_mean,'ko','markersize',6,'markerfacecolor','r','linewidth',lw);
% observed melt
errorbar(ax(6),t_melt,m,m_ci_l,m_ci_h,'.','linewidth',1.2*lw,'color','k','capsize',10)
h6(2) = plot(ax(6),t_melt,m,'k^','markersize',8,'linewidth',0.5*lw,'markerfacecolor',dep_clrs(3,:));

% T
hold(ax(4),'on')
plotConfidenceRegion(ax(4),extrema(T(1).time),T_outer(1)*[1 1],T_outer(2)*[1 1],T_clrs(2,:),0.6);
for i = 1:3
    plot(ax(4),T(i).time,T(i).values,'-','color',T_clrs(4-i,:),'linewidth',lw);
end
for i = 1:3
...
plot(ax(4),t_melt,T_seg,'k^','markersize',8,'linewidth',0.5*lw,'markerfacecolor',colors(3));
% plot(ax(2),T(1).time,T(2).values,'-','color',T_clrs(4-3,:),'linewidth',lw)

% S
hold(ax(5),'on')
plotConfidenceRegion(ax(5),extrema(hobo.time),S_outer(1)*[1 1],S_outer(2)*[1 1],0.7*vel_clrs(3,:),0.6);
plot(ax(5),hobo.time,hobo.S,'-','color',0.7*vel_clrs(3,:),'linewidth',1.2*lw);
plot(ax(5),t_melt,S_seg,'k^','markersize',8,'linewidth',0.5*lw,'markerfacecolor',colors(5));

% vel mag
hold(ax(3),'on')
plot(ax(3),adcp.burst.time,hannFilter(umax,8*2),'-','color',0.2*[1 1 1],'linewidth',lw)
plot(ax(3),t_melt,u_seg,'k^','markersize',8,'linewidth',0.5*lw,'markerfacecolor',colors(6));
set(ax(3),'fontsize',fs)

% vel pcolors
pcolor(ax(1),adcp.burst.time,y,uvw(:,:,1)')
pcolor(ax(2),adcp.burst.time,y,uvw(:,:,3)')
for i = 1:2
    shading(ax(i),'flat')
    cmocean('bal',ax(i))
    clim(ax(i),0.4*[-1 1])
...
ylabel(ax(4),'T [\circC]','fontsize',fs,'fontweight','bold')
ylabel(ax(5),'S [psu]','fontsize',fs,'fontweight','bold')
ylabel(ax(3),'speed [m/s]','fontsize',fs,'fontweight','bold')
ylabel(ax(6),'melt [m/day]','fontsize',fs,'fontweight','bold')
...
p_lbls(3).Position(2) = 1.22;
p_lbls(3).String = 'sqrt(u^2+w^2)';
p_lbls(1).String = 'u (horizontal along-ice)';
% p_lbls(5).String = 'v (horz. ice-normal)';
p_lbls(2).String = 'w (vertical)';
p_lbls(4).String = 'temperature';
p_lbls(5).String = 'salinity';
p_lbls(6).String = 'melt rate';

% colobar
cbar = colorbar(ax(2),'position',cbarpos(ax(1:2),.005,.015));
cbar.Label.String = '[m/s]';
cbar.Label.FontSize = fs;
cbar.Label.FontWeight = 'bold';
cbar.FontSize = fs-1;
...
lgd4 = legend(ax(4),h4,{'10cm','25cm','40cm'},'fontsize',fs,'fontweight','bold');
lgd4.Position(1) = sum([ax(4).Position([1 3]) .005]);
