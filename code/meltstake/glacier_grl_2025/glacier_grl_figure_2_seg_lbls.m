...
% m_3eqn,coarse. BUT NOW WITH BIG SEGMENT LABELS!!!
%
% KJW
% 24 Oct 2025
...
% make a list of segments for glacier deployments
seg_lbls = cell(sum(any(idx_glac,2)),1);
pos = 1;
for i = 1:3
    for j = 1:sum(idx_glac(:,i))
        seg_lbls{pos} = num2str(j);
        pos = pos + 1;
    end
end
...
x_txt = u(any(idx_glac,2))+.005;
y_txt = m(any(idx_glac,2));
for i = 1:length(seg_lbls)
    text(ax(1),x_txt(i),y_txt(i),seg_lbls{i},'horizontalalignment','left','fontsize',fs-2)
end
...
x_txt = mod_glac.m_mean(:,1)+.05;
y_txt = m(any(idx_glac,2));
for i = 1:length(seg_lbls)
    text(ax(2),x_txt(i),y_txt(i),seg_lbls{i},'horizontalalignment','left','fontsize',fs-2)
end
...
x_txt = mod_glac.m_mean(:,5)+.05;
y_txt = m(any(idx_glac,2));
for i = 1:length(seg_lbls)
    text(ax(3),x_txt(i),y_txt(i),seg_lbls{i},'horizontalalignment','left','fontsize',fs-2)
end
