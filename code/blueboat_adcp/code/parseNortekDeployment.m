...
if ~exist(fullfile(depname,'Nortek'),'dir')
    warning('directory does not exist: %s\n',fullfile(depname,'Nortek'));
    return
end

%%% Look for instrument.txt file %%% (I've commented this out since it's
% specific to my parsing workflow --KJW)
if false % exist(fullfile(depname,'Nortek','instruments.txt'),'file')
    info = parseInstrumentFile(depname,'Nortek');
    fprintf('instrument file found, using time offset dt=%.1f\n',seconds(info.t_offset));
    if ~isnan(info.t_offset)
        dt = seconds(info.t_offset);
    else
        dt = seconds(0);
    end
    use_inst_file = true;
...
%%% Files %%%
d = dir(fullfile(depname,'Nortek','*.mat'));
fnames = {d.name}';
nf = length(fnames);
if nf == 0
...
%%% Instrument config %%%
cfg = load(fullfile(depname,'Nortek',fnames{1}),'Config'); cfg = cfg.Config;
adcp.cfg = struct();
...
for i = 1:nf
    fprintf('%d/%d\n',i,nf);
    % load MIDAS MAT file
    data = load(fullfile(depname,'Nortek',fnames{i}),'Data'); data = data.Data;
    if i == 1
